...
class LocalProfilePresenterTest {
    ...
    fun loadPublishesSharedDataAndClearsLoadingState() = runTest {
        ...
        presenter.load(object : LocalProfileDataRepository {
            override suspend fun load(libraryEntries: List<org.akkirrai.hibiki.library.LibraryEntry>?): LocalProfileData = expected
        })
    ...
    fun failedLoadDoesNotLeavePresenterLoading() = runTest {
        ...
        runCatching {
            presenter.load(object : LocalProfileDataRepository {
                override suspend fun load(libraryEntries: List<org.akkirrai.hibiki.library.LibraryEntry>?): LocalProfileData = error("load failed")
            })
        }
