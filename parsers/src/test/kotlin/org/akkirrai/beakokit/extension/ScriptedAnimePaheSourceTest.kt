...
/**
 * Proves the Rhino-scripted AnimePahe extension against the same fixtures the old compiled-in
 * AnimePaheSource used. Every fixture route responds 200 directly (no 403/cf-mitigated header),
 * so - exactly like the original Kotlin test - the Cloudflare-challenge retry path
 * (`fetchChallenged`/`challenge()`) is never actually exercised here; see
 * [RhinoExtensionRuntimeChallengeTest] for a focused test of that mechanism itself.
 *
 * The extension payload (`animepahe.js`) is a gitignored local-only fixture - these tests skip
 * themselves when it's absent instead of failing.
 */
class ScriptedAnimePaheSourceTest {
    @BeforeTest
    fun requireLocalFixture() {
        assumeTrue(ScriptedExtensionFixtures.isAvailable("animepahe"), "animepahe.js fixture is not present locally")
    }
    ...
    fun `English API catalog and Kwik playback satisfy shared contracts`() = runBlocking {
        SourceFixtureHost(
            ...
            val source = ScriptedAnimeSource(host.context, ScriptedExtensionFixtures.load("animepahe"))
