...
/**
 * Proves the Rhino-scripted AnimeGo extension against the same fixtures the old compiled-in
 * AnimeGoSource used. The search-description enrichment (originally 4-way concurrent) runs
 * sequentially here since Rhino's `fetch` is synchronous - same result, verified by the same
 * assertion the old test used (a single search result gets its description backfilled from
 * its details page).
 *
 * The extension payload (`animego.js`) is a gitignored local-only fixture - these tests skip
 * themselves when it's absent instead of failing.
 */
class ScriptedAnimeGoSourceTest {
    @BeforeTest
    fun requireLocalFixture() {
        assumeTrue(ScriptedExtensionFixtures.isAvailable("animego"), "animego.js fixture is not present locally")
    }
    ...
    fun `scraped catalog and playback satisfy shared contracts`() = runBlocking {
        SourceFixtureHost(
            routes = listOf(
                ...
                FixtureRoute.fromResource(path = "/anime/2", resource = "beakokit/animego/catalog-page.json"),
                ...
                FixtureRoute.fromResource(path = "/player/710", resource = "beakokit/animego/episodes.json"),
                FixtureRoute.fromResource(path = "/player/videos/10934", resource = "beakokit/animego/players.json"),
            ),
            preferredLanguages = listOf(SourceLanguage.RUSSIAN),
        ).use { host ->
            val source = ScriptedAnimeSource(host.context, ScriptedExtensionFixtures.load("animego"))
            ...
            val paged = source.search(AnimeSearchRequest(offset = 20, limit = 1, sort = AnimeSearchSort.RATING))
            ...
            SourceTestKit.assertEmptySearchContract(source, AnimeSearchRequest(query = "Missing", limit = 1))
