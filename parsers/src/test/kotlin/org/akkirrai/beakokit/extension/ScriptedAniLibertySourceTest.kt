...
/**
 * Proves the Rhino-scripted AniLiberty extension against the same fixtures the old compiled-in
 * AniLibertySource used, reusing them unchanged. This port skips the original's cross-provider
 * title-name re-search for playback (`TitleMatcher`/`ProviderMatch`) since catalog and playback
 * are the exact same site/API and the catalog id is already a valid release id (confirmed by
 * these fixtures using the same id "987654" for both catalog and playback).
 *
 * The extension payload (`aniliberty.js`) is a gitignored local-only fixture - these tests skip
 * themselves when it's absent instead of failing.
 */
class ScriptedAniLibertySourceTest {
    @BeforeTest
    fun requireLocalFixture() {
        assumeTrue(ScriptedExtensionFixtures.isAvailable("aniliberty"), "aniliberty.js fixture is not present locally")
    }
    ...
    fun `catalog operations satisfy the shared contract`() = runBlocking {
        SourceFixtureHost(
            ...
            val source = ScriptedAnimeSource(host.context, ScriptedExtensionFixtures.load("aniliberty"))
